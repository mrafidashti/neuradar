...
def calculate_radar_loss(
    batch: Tensor, prediction: Tensor, indices: Tensor, loss_type: str = "nll", training: bool = True
):
    ...
    for i in range(num_scans):
        ...
        association[torch.tensor(row_ind, dtype=torch.long, device=gt.device), 1] = torch.tensor(
            col_ind, device=gt.device
        ).float()
...
def get_radar_loss(batch, mb, association, loss_type):
    ...
    if loss_type == "nll":
        ...
        if associated.sum() > 0:
            ...
            associated_costs = (
                -x_log_probs[associated, ...] - y_log_probs[associated, ...] - z_log_probs[associated, ...]
            )
            ...
            losses[associated] = (
                associated_costs[torch.arange(associated_costs.shape[0]), association[associated, 1].long()]
                - log_r_k_loss[associated]
            )
        ...
        if associated.sum() > 0:
            ...
            losses[associated] += torch.norm(
                cart_params[association[associated, 0].long(), :] - batch[association[associated, 1].long(), :3], dim=-1
            ).flatten()
...
def sample_radar_points(
    radar_output: Tensor, loss_type: str, threshold: Optional[float] = 0.5, max_detections: int = 1000
):
    ...
    if loss_type == "nll":
        ...
        for i in range(mb.n_mb):
            ...
            if random_prob < mb.existence_probabilities[..., i] and i in sorted_ep:
                ber_indices = torch.cat(
                    [ber_indices, torch.tensor([i], device=mb.existence_probabilities.device).long()]
                )
                x = torch.cat(
                    [x, mb.distribution(loc=mb.x[..., i], scale=mb.x_var[..., i]).rsample(torch.Size([1])).view(-1, 1)]
                )
                y = torch.cat(
                    [y, mb.distribution(loc=mb.y[..., i], scale=mb.y_var[..., i]).rsample(torch.Size([1])).view(-1, 1)]
                )
                ...
                if mb.z_var is not None:
                    z = torch.cat(
                        [
                            z,
                            mb.distribution(loc=mb.z[..., i], scale=mb.z_var[..., i])
...
def plot_radar_samples(batch, radar_points, association, mb, ber_indices):
    ...
    if batch is not None:
        ...
        fig.add_trace(
            go.Scatter3d(
                x=batch[..., 0].squeeze().tolist(),
                y=batch[..., 1].squeeze().tolist(),
                z=batch[..., 2].squeeze().tolist(),
                mode="markers",
                marker=dict(color="green", size=[10], sizemode="diameter"),
                name="Batch",
            )
        )
    ...
    if association is not None:
            ...
            if association[b, 1] > -1:
                fig.add_trace(
                    go.Scatter3d(
                        ...
                        y=[radar_points[b, 1]],
                        z=[radar_points[b, 2]],
                        opacity=(existence_probabilities[index]).item(),
                        mode="markers",
                        marker=dict(color="blue", size=[10], sizemode="diameter"),
                        name=f"Radar Point {index}",
                    )
                )
            else:
                fig.add_trace(
                    go.Scatter3d(
                        x=[radar_points[b, 0].tolist()],
                        y=[radar_points[b, 1].tolist()],
                        z=[radar_points[b, 2].tolist()],
                        opacity=(existence_probabilities[index]).item(),
                        mode="markers",
                        marker=dict(color="red", size=[10], sizemode="diameter"),
                        name=f"Radar Point {index}",
                    )
                )
        ...
        fig.add_trace(
            go.Scatter3d(
                x=radar_points[..., 0].squeeze(),
                y=radar_points[..., 1].squeeze(),
                z=radar_points[..., 2].squeeze(),
                mode="markers",
                marker=dict(color="blue", size=[10], sizemode="diameter"),
                name="Radar points",
            )
