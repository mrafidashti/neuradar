...
class RadarRayGenrator(nn.Module):
    """torch.nn Module for generating rays for Radars."""

    def __init__(self, radars: Radars) -> None:
        super().__init__()
        self.radars = radars

    def forward(self, scan_indices: Optional[Int[Tensor, "n_scans"]] = None) -> RayBundle:
        """Index into the radars to generate the rays.

        Args:
            ray_indices: Contains (radar_scan_idx, point_local_idx) for target rays.
        """

        ray_bundle = self.radars.generate_rays(scan_indices=scan_indices)
