...
from zod.constants import EGO as ZOD_EGO, Radar as ZodRadar
from zod.data_classes.box import Box3D
from zod.data_classes.sensor import LidarData, RadarData
...
class Zod(ADDataParser):
    ...
    def _get_radars(self) -> Tuple[Radars , List[str]]:
        ...
        radar_frames = self.zod_seq.info.get_radar_frames(radar=ZodRadar.FRONT)
        frame = radar_frames[0]
        radar_filename = self.config.data / radar_frames[0].filepath
        frame.filepath = str(radar_filename)
        timestamps = np.unique(frame.read().timestamp) / 1e9  # convert to seconds

        poses = []
        for timestamp in timestamps:
            ego_pose = self.zod_seq.oxts.get_poses(timestamp)
            radar_pose = self.zod_seq.calibration.radars[ZodRadar.FRONT].extrinsics.transform.copy()
        ...
        radars = Radars(
            ...
            radar_type=RadarType.ContiFLR2,
    ...
    def _read_radars(self, radars: Radars, filenames: List[Path]) -> List[torch.Tensor]:
        """Reads radar point clouds from the given filenames."""

        times = radars.times
        radar_data = RadarData.from_npy(str(filenames[0]))
        data_timestamps = radar_data.timestamp / 1e9  # convert to seconds
        time_indices = np.searchsorted(data_timestamps, times.numpy().flatten())
        scan_indices = np.unique(radar_data.scan_index[time_indices])
        cartesian_coordinates = radar_data.get_cartesian_coordinates()

        radar_pcs = []
        for idx in scan_indices:
            indices = np.where(radar_data.scan_index == idx)[0]
            quality = radar_data.quality[indices]
            snrs = radar_data.amplitude[indices]
            range_rates = radar_data.range_rate[indices]
            mode = radar_data.mode[indices]
            xyz = cartesian_coordinates[indices, :]

            valid_indices = quality < 3  # the quality of detections could be 0, 1, or 2 (bad, medium, good)
            cloud = np.stack(
                [
                    xyz[:, 0],
                    xyz[:, 1],
        ...
        radar_pcs = [torch.tensor(pc, dtype=torch.float32) for pc in radar_pcs]
