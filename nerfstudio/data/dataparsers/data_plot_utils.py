...
def plot_data_for_iteration(
    ...
    # Get point clouds
    lidar_pc = (
        transform_points_pairwise(lidar_pointclouds[index][:, :3].float(), lidars.lidar_to_worlds[index]).cpu().numpy()
    )
    ...
    fig.add_trace(
        go.Scatter3d(
            ...
            z=lidar_pc[:, 2],
            mode="markers",
            marker=dict(size=0.4, color="blue"),
            name="lidar",
        )
    )
    ...
    fig.add_trace(
        go.Scatter3d(x=[0], y=[0], z=[0], mode="markers", marker=dict(size=2, color="magenta"), name="coords origin")
    )
    ...
    radial_unit_vectors = (radar_pc[n_pcs] - origin_radar) / np.linalg.norm(
        radar_pc[n_pcs] - origin_radar, axis=1, keepdims=True
    )
    ...
    for i in range(np.shape(radial_velocity_vectors)[0]):
        ...
        fig.add_trace(
            go.Scatter3d(
                x=[curr_pc[0], vel_pc[0]],
                y=[curr_pc[1], vel_pc[1]],
                z=[curr_pc[2], vel_pc[2]],
                mode="lines",
                line=dict(color=colors[n_pcs]),
                name=f"radar {n_pcs} velocity {i}",
            )
        )
    ...
    if render:
        ...
        fig.add_trace(
            go.Scatter3d(
                x=sampled_points[indices, 0],
                y=sampled_points[indices, 1],
                z=sampled_points[indices, 2],
                mode="markers",
        ...
        radial_unit_vectors = (sampled_points[:, :3] - origin_radar) / np.linalg.norm(
            sampled_points[:, :3] - origin_radar, axis=1, keepdims=True
        )
        ...
        for i in range(np.shape(radial_velocity_vectors)[0]):
            fig.add_trace(
                go.Scatter3d(
                    x=[sampled_points[i, 0], sampled_points[i, 0] + radial_velocity_vectors[i, 0]],
                    y=[sampled_points[i, 1], sampled_points[i, 1] + radial_velocity_vectors[i, 1]],
                    z=[sampled_points[i, 2], sampled_points[i, 2] + radial_velocity_vectors[i, 2]],
                    mode="lines",
