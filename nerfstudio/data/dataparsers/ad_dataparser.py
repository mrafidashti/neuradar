...
class ADDataParser(DataParser):
    ...
    def _get_radars(self) -> Tuple[Radars, List[Path]]:
    ...
    def _generate_dataparser_outputs(self, split="train"):
        ...
        lidars, lidar_filenames = self._get_lidars() if self.config.lidars else (_empty_lidars(), [])
        radars, radar_filenames = self._get_radars() if self.config.radars else (_empty_radars(), [], [])
        ...
        lidar_pcs = self._read_lidars(lidars, lidar_filenames)
        radar_pcs = self._read_radars(radars, radar_filenames)

        # Process the data
        self._remove_ego_points(lidar_pcs)
        time_offset = self._adjust_times(cameras, lidars, lidar_pcs, radars, radar_pcs, trajectories)
        dataparser_transform = self._adjust_poses(cameras, lidars, radars, trajectories)
        scene_box = self._compute_scene_box(cameras, lidars)
        self._add_sensor_velocities(cameras, lidars, radars)

        # Adjust sensor idxs to avoid overlap (if necessary)
        cam_sensor_idxs = cameras.metadata["sensor_idxs"].unique()
        lidar_sensor_idxs = lidars.metadata["sensor_idxs"].unique()
        ...
        lidars, lidar_pcs = lidars[lidar_idxs], [lidar_pcs[i] for i in lidar_idxs]
        radar_idxs = self._get_train_eval_indices(radars)[split]
        radars, radar_pcs = radars[radar_idxs], [radar_pcs[i] for i in radar_idxs]
        ...
        point_clouds_rgb = (
            None if not self.config.paint_points else _paint_points(lidars, lidar_pcs, cameras, img_filenames)
        )
        ...
        return DataparserOutputs(
            ...
            metadata={
                "lidars": lidars,
                "point_clouds": [pc.float() for pc in lidar_pcs],  # Ensure they are float32
                "radars": radars,
                "radar_pcs": radar_pcs,
    ...
    def _filter_based_on_time(
        ...
        lidars: Lidars,
        lidar_filenames: List[Path],
        radars: Radars,
        radar_filenames: List[Path],
        trajectories: List[Dict],
    ) -> Tuple[Cameras, List[Path], Lidars, List[Path], Radars, List[Path], List[Dict]]:
        ...
        lidars, lidar_filenames = _filter_sensordata_on_time(lidars, lidar_filenames, start_time, end_time)
        radars, radar_filenames = _filter_sensordata_on_time(radars, radar_filenames, start_time, end_time)
        assert len(cameras) or len(lidars) or len(radars), "No cameras, lidars, or radars in the dataset"
    ...
    def _adjust_times(
        ...
        radars.times = (radars.times - min_time).float()
        for traj in trajectories:
    ...
    def _adjust_poses(self, cameras: Cameras, lidars: Lidars, radars: Radars, trajectories: List[Dict]):
        ...
        radars.radar_to_worlds = pose_multiply(w2m, radars.radar_to_worlds).to(torch.float32)
        for traj in trajectories:
    ...
    def _get_linspaced_indices(self, sensor_idxs: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        if sensor_idxs.numel() == 0:
            ...
            # we will use this for both train and val, as we do not mean to use the validation performance for anything
    ...
    def _add_sensor_velocities(self, cameras: Cameras, lidars: Lidars, radars: Radars):
    ...
    def _get_missing_points(
        ...
        for channel_id, expected_elevation in self.config.lidar_elevation_mapping[lidar_name].items():
            if self.config.skip_elevation_channels is not None:
...
def _filter_sensordata_on_time(
    ...
    """Filter the data based on start and end time."""
