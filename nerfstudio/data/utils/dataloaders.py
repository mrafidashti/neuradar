...
class EvalDataloader(DataLoader):
    ...
    def __init__(
        self,
        dataset: Union[InputDataset, LidarDataset, RadarDataset],
        device: Union[torch.device, str] = "cpu",
        **kwargs,
    ):
        self.dataset = dataset
        ...
        elif isinstance(dataset, LidarDataset):
            self.sensors = dataset.lidars.to(device)
        elif isinstance(dataset, RadarDataset):
            self.sensors = dataset.radars.to(device)
        self.device = device
        self.kwargs = kwargs
    ...
    def get_camera(self, image_idx: int = 0) -> Tuple[Union[Cameras, Lidars, Radars], Dict]:
