...
@dataclass
class RadarPointSamplerConfig(PixelSamplerConfig):
    """Config dataclass for RadarPointSampler."""

    _target: Type = field(default_factory=lambda: RadarPointSampler)
    """Target class to instantiate."""
...
class RadarPointSampler(PixelSampler):
    ...
    def collate_image_dataset_batch(self, batch: Dict):
        ...
        Args:
            ...
            keep_full_image: whether or not to include a reference to the full image in returned batch
        """
        n_scans = self.config.num_radar_scans_per_batch
        device = batch["radar"].device
        num_radars = len(batch["points_per_radar"])
        if num_radars <= 0:
            return None, None
        elif num_radars <= n_scans:
        ...
        c, n = (i.flatten() for i in torch.split(indices_v2, 1, dim=-1))
        collated_batch = {
            key: value[c, n]
            for key, value in batch.items()
            if key not in ("radar", "radar_idx", "points_per_radar", "sensor_idx") and value is not None
        }
    ...
    def sample(self, radar_batch: Dict):
        """Sample a radar batch and return a point batch.

        Args:
            radar_batch: batch of -images- lidars to sample from
        """
        if isinstance(radar_batch["radar"], torch.Tensor):
            point_batch, indices = self.collate_image_dataset_batch(radar_batch)
        else:
            raise ValueError("radar_batch['radar'] must be a torch.Tensor")
        return point_batch, indices
