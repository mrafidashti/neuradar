...
from nerfstudio.data.datamanagers.ad_neuradar_datamanager import ADNeuRadarDataManagerConfig
...
descriptions = {
    ...
    "neuradar": "NeuRadar supporting radar data",
    "neurader": "Bigger, better, longer, stronger NeuRAD",
}
...
method_configs["neuradar"] = TrainerConfig(
    method_name="neuradar",
    steps_per_eval_batch=500,
    steps_per_eval_image=2000,
    steps_per_eval_all_images=20000,
    steps_per_eval_all_radars=20000,
    steps_per_save=10000,
    max_num_iterations=20001,
    mixed_precision=True,
    pipeline=ADNeuRadarPipelineConfig(
        calc_fid_steps=(99999999,),
        datamanager=ADNeuRadarDataManagerConfig(dataparser=ZodDataParserConfig(add_missing_points=True)),
        model=NeuRadarModelConfig(
            eval_num_rays_per_chunk=1 << 15,
            camera_optimizer=CameraOptimizerConfig(mode="off"),  # SO3xR3
        ),
    ),
    optimizers={
        ...
        "transformer": {
            "optimizer": AdamWOptimizerConfig(lr=1e-3, eps=1e-15, weight_decay=1e-7),
            "scheduler": ExponentialDecaySchedulerConfig(lr_final=1e-7, max_steps=10001, warmup_steps=5000),
        },
    },
    viewer=ViewerConfig(num_rays_per_chunk=1 << 15),
    vis="viewer",
