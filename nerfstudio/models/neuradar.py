...
"""
NeuRadar (Neural Rendering for Autonomous Driving) model.
"""
...
class LossSettings:
    ...
    # Radar losses
    radar_mult: float = 0.02
    """Multiplier for the negative log likelihood loss."""
    radar_loss_type: Literal["nll", "euclidean"] = "nll"
    """Type of radar loss to use."""


@dataclass
class SamplingSettings:
    """Configuration of NeuRadar proposal sampling."""
...
@dataclass
class NeuRadarModelConfig(ADModelConfig):
    """Overall configuration of NeuRadar."""

    _target: Type = field(default_factory=lambda: NeuRadarModel)
    ...
    existence_probability_threshold: float = 0.5
    """Existence probability threshold for sampling radar points in deterministic method."""
...
class NeuRadarModel(ADModel):
    """NeuRadar model.

    Args:
        config: NeuRadar configuration to instantiate model
    """

    config: NeuRadarModelConfig
    ...
    def populate_modules(self):
        ...
        self.n_features = self.config.field.nff_out_dim + self.config.appearance_dim
        self.radar_decoder = Transformer(d_model=self.n_features)
        ...
        self.radar_uncertainty_head = MLP(
            in_dim=self.n_features,
            layer_width=16,
            out_dim=3,
            num_layers=3,
        ...
        self.existence_probability_head = MLP(
            in_dim=self.n_features,
            layer_width=16,
            out_dim=1,
            num_layers=3,
    ...
    def get_param_groups(self) -> Dict[str, List[Parameter]]:
        ...
        # radar components
        param_groups["transformer"] += list(self.radar_decoder.parameters())
        param_groups["transformer"] += list(self.offset_head.parameters())
        param_groups["transformer"] += list(self.radar_angle_head.parameters())
        param_groups["transformer"] += list(self.radar_uncertainty_head.parameters())
        param_groups["transformer"] += list(self.existence_probability_head.parameters())
    ...
    def forward(
        ...
        num_radar_scans = batch["radar_scan_indices"].size(0)
        return self.get_outputs(ray_bundle, patch_size, num_radar_scans, intensity_for_cam, calc_lidar_losses)

    def get_outputs(
        self,
        ray_bundle: RayBundle,
        patch_size: Tuple[int, int],
        num_radar_scans: Optional[int] = 1,
        intensity_for_cam: bool = False,
        calc_lidar_losses: bool = True,
    ):
        if self.training:
        ...
        rgb, intensity, ray_drop_logits, radar_output = self.decode_features(
            features=nff_outputs["features"],
            patch_size=patch_size,
            depth=nff_outputs["depth"],
            directions_spher=ray_bundle.metadata.get("directions_spher"),
            intensity_for_cam=intensity_for_cam,
            is_lidar=ray_bundle.metadata.get("is_lidar"),
            is_radar=ray_bundle.metadata.get("is_radar"),
            num_radar_scans=num_radar_scans,
        )
        ...
        if radar_output is not None:
            nff_outputs["radar_output"] = radar_output
        return nff_outputs
    ...
    def decode_features(
        ...
        patch_size: Tuple[int, int],
        depth: Tensor,
        directions_spher: Tensor,
        intensity_for_cam: bool = False,
        is_lidar: Optional[Tensor] = None,
        is_radar: Optional[Tensor] = None,
        num_radar_scans: Optional[int] = None,
        ...
        # Decode radar features
        radar_output = None
        if is_radar is not None and radar_features.numel() > 0:
            # N: num_scans or batch size; nr: num_rays; C: num_channels/features
            ...
            radar_xyz = pos_embedding_input[..., :3] + offset  # (x, y, z) offsets, (N, nr, 3)
            ep = self.existence_probability_head(final_output)  # existence probability, (N, nr, 1)
            radar_unc = self.radar_uncertainty_head(final_output)  # (x, y, z) variances, (N, nr, 3)
            radar_output = torch.cat((ep, radar_xyz, radar_unc), dim=-1).type(torch.float32)

        return rgb, intensity, ray_drop_logit, radar_output
    ...
    def get_metrics_dict(self, outputs, batch):
        ...
        if "radar" in batch:
            batch["radar"] = batch["radar"].to(self.device)
        ...
        return metrics_dict, radar_dict
    ...
    def get_loss_dict(self, outputs, batch, metrics_dict=None):
        ...
        if self.training:
            ...
            if "radar_loss" in metrics_dict:
                loss_dict["radar_loss"] = conf.radar_mult * metrics_dict["radar_loss"]
        return loss_dict
    ...
    def get_image_metrics_and_images(
        ...
        if "radar" in batch:
            if "is_radar" not in batch:
                batch["is_radar"] = torch.ones(*batch["radar"].shape[:-1], 1, dtype=torch.bool, device=self.device)
            ...
            if (
                ...
                metrics_dict["emd_distance_radar"] = dist
            elif radar_points_metrics.any() and radar_points_metrics.shape[0] > 0:
                dist = radar_points_metrics.norm(dim=-1).mean()
                metrics_dict["chamfer_distance_radar"] = dist
                metrics_dict["emd_distance_radar"] = dist
            else:
                metrics_dict["chamfer_distance_radar"] = 0.0
            ...
            if time is not None and r2w is not None and radar_idx is not None:
                ...
                actor_information = {
                    ...
                    "actor_indices": actor_indices.to(self.device),
                    "actor_sizes": actor_sizes.to(self.device),
                    "scan_indices": scan_indices.to(self.device),
                }
                radar_figure = add_actor_boxes_to_figure(radar_figure, actor_information, r2w, radar_idx)

            radar_figure.update_layout(title="Radar Detections")
            radar_dict["radar_pc_figure"] = radar_figure

        return metrics_dict, images_dict, radar_dict

    def get_radar_metrics(
        self, outputs: Dict[str, torch.Tensor], batch: Dict[str, torch.Tensor], sampling_rounds: int = 10
        ...
        if "radar" in batch:
            ...
            for i in range(sampling_rounds):
                ...
                if (
                    ...
                    metrics_dict["emd_distance"][i] = dist
                elif radar_points_metrics.any() and radar_points_metrics.shape[0] > 0:
                    dist = torch.mean(torch.norm(radar_points_metrics, dim=-1)).float()
                    metrics_dict["chamfer_distance"][i] = dist
                    metrics_dict["emd_distance"][i] = dist
                else:
                    metrics_dict["chamfer_distance"][i] = 0.0
                    metrics_dict["emd_distance"][i] = 0.0
    ...
    def get_outputs_for_camera_ray_bundle(self, camera_ray_bundle: RayBundle) -> Dict[str, torch.Tensor]:
        ...
        """
        if len(camera_ray_bundle.shape) == 1:  # lidar or radar
            output_size, patch_size = (camera_ray_bundle.shape[0],), (1, 1)
            if "is_lidar" in camera_ray_bundle.metadata:
                is_lidar = torch.ones_like(camera_ray_bundle.pixel_area, dtype=torch.bool)
                is_radar = torch.zeros_like(camera_ray_bundle.pixel_area, dtype=torch.bool)
            if "is_radar" in camera_ray_bundle.metadata:
                is_lidar = torch.zeros_like(camera_ray_bundle.pixel_area, dtype=torch.bool)
                is_radar = torch.ones_like(camera_ray_bundle.pixel_area, dtype=torch.bool)
        else:  # camera
        ...
        if is_radar is not None and is_radar.any():
            num_rays_per_chunk = num_rays
        outputs_lists = defaultdict(list)
        ...
        if radar_output is not None:
            outputs["radar_output"] = radar_output
    ...
    def _scale_pixel_area(self, ray_bundle: RayBundle):
        ...
        if is_lidar is not None and is_radar is not None:
            scaling[~(is_lidar | is_radar)] = self.config.rgb_upsample_factor**2
        elif is_lidar is not None and is_radar is None:
            scaling[~is_lidar] = self.config.rgb_upsample_factor**2
        elif is_radar is not None and is_lidar is None:
            scaling[~is_radar] = self.config.rgb_upsample_factor**2
        else:
    ...
    def _get_cartesian_coords(self, depth, theta, phi):
        x_pos = depth * torch.cos(phi) * torch.cos(theta)
        y_pos = depth * torch.sin(phi) * torch.cos(theta)
        z_pos = depth * torch.sin(theta)
