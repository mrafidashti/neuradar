...
class ADModel(Model):
    ...
    # @torch.no_grad()
    # def get_outputs_for_radar(
    #     self, radar: Radars, batch: Dict[str, torch.Tensor]
    # ) -> Tuple[Dict[str, torch.Tensor], Dict[str, torch.Tensor]]:
    #     """Takes in a radar, generates the raybundle, and computes the output of the model.
    #     Assumes a ray-based model.
    ...
    #     ray_bundle = radar.generate_rays(radar_indices=ray_indices, scan_indices=scan_index).to(self.device)  # Using "self.eval_radar_dataset.radars" is silly
