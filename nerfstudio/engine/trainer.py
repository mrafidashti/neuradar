...
class TrainerConfig(ExperimentConfig):
    ...
    steps_show_radar_train_figure: int = 1000
    """Number of steps between showing the predicted radar pc during training"""
    steps_per_eval_all_radars: int = 1000
    """Number of steps between eval all radars."""
...
class Trainer:
            ...
            for step in range(self._start_step, self._start_step + num_iterations):
                ...
                if self.pipeline.datamanager.eval_dataset:
                    with self.train_lock:  # avoid race conditions with eval
                        if isinstance(self.pipeline, ADNeuRadarPipeline):
                            self.eval_iteration_neuradar(step)
                        else:
    ...
    def train_iteration(self, step: int) -> TRAIN_INTERATION_OUTPUT:
        ...
        with torch.autocast(device_type=cpu_or_cuda_str, enabled=self.mixed_precision):
            if isinstance(self.pipeline, ADNeuRadarPipeline):
                _, loss_dict, metrics_dict, radar_dict = self.pipeline.get_train_loss_dict(step=step)
                if step % self.config.steps_show_radar_train_figure == 0:
                    writer.put_figure(name="Train Radar" + "/" + "Radar Point Cloud", figure_dict=radar_dict, step=step)
            else:
    ...
    @profiler.time_function
    @torch.no_grad()
    def eval_iteration_neuradar(self, step: int) -> None:
        """Run one iteration with different batch/image/all image evaluations depending on step size.
        ...
        # a batch of eval rays
        if step_check(step, self.config.steps_per_eval_batch):
            _, eval_loss_dict, eval_metrics_dict, radar_dict = self.pipeline.get_eval_loss_dict(step=step)
            eval_loss_dict = detach_items_if_tensor(eval_loss_dict)
            eval_metrics_dict = detach_items_if_tensor(eval_metrics_dict)
            eval_loss = functools.reduce(torch.add, eval_loss_dict.values())
            writer.put_scalar(name="Eval Loss", scalar=eval_loss, step=step)
        ...
        if step_check(step, self.config.steps_per_eval_image):
            with TimeWriter(writer, EventName.TEST_RAYS_PER_SEC, write=False) as test_t:
                metrics_dict, images_dict, radar_dict = self.pipeline.get_eval_image_metrics_and_images(step=step)
            writer.put_time(
                name=EventName.TEST_RAYS_PER_SEC,
                duration=metrics_dict["num_rays"] / test_t.duration,
                step=step,
            ...
            radar_group = "Eval Radar"
            writer.put_figure(name=radar_group + "/" + "Radar Point Cloud", figure_dict=radar_dict, step=step)
